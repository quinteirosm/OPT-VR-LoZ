...
<template>
  <a-entity :id="`portal-${id}`" geometry="primitive: box; radius: 1" clickable :teleport-camera-rig="`
      x: ${x};
            y: ${y};
            z: ${z};
    ...
    <Teleport to="a-scene" v-if="cameraEffect">
      <a-entity :rotation="`0 ${cameraRot} 0`">
        <a-entity :id="`camera-${id}`" :secondary-camera="`
            ...
            sequence: before;
          `" :position="`${cameraX} ${cameraY} ${cameraZ}`" :bind-rotation="`target: ${cameraSelector}`"></a-entity>
      </a-entity>
    </Teleport>
